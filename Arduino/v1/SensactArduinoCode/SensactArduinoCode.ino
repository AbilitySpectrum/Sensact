...
long whenOn[nOutputs], control[nOutputs] = {
  0, SENSACT_OUT1, SENSACT_OUT2, 0, 0, SENSACT_RED, 0, SENSACT_BUZZER
}; // ylh WHY????
long onOff[nOutputs];
...
#ifdef INCLUDE_MPU6050 || INCLUDE_LGGYRO
    Serial.print(gyro_read());
    Serial.print(",");
    Serial.print(constrain(map( my_gyro.g.x, -31000, 31000, 0, 100 ), 0, 100));
    Serial.print(",");
    Serial.print(constrain(map( my_gyro.g.y, -31000, 31000, 0, 100 ), 0, 100));
    Serial.print(",");
    Serial.print(constrain(map( my_gyro.g.z , -31000, 31000, 0, 100 ), 0, 100));
    Serial.print("\n");
#endif
    ...
    if ( val > config[ offset2 + THRESHOLD_VAL] ) {
      ...
      for ( int j = 0; j < nOutputs; j++ ) {
        switch ( j ) {
          ...
          case BUZZER:
            if ( config[offset + j] ) {
              tone(control[j], NOTE_C5, 30); // 250); //digitalWrite(control[j], HIGH);
              startedOutput(j);
            }
            break;
          case CLICK:
